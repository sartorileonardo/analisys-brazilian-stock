...
class StockBusiness(val acaoConfig: StockParametersApiConfig) {
    ...
    fun getAnalise(ticker: String): Mono<StockAnalysisDto> {
        ...
        val dividaLiquidaSobrePatrimonioLiquido =
            extrairDouble(company.get("dividaliquida_PatrimonioLiquido").toString())
    ...
    private fun possuiBomNivelFreeFloat(freeFloat: Double): Boolean =
        freeFloat.compareTo(acaoConfig.minimoFreeFloat.toDouble()) >= 1

    private fun possuiBomNivelCrescimentoLucroNosUltimos5Anos(cagr: Double): Boolean =
        cagr.compareTo(acaoConfig.minimoCagrLucro5anos.toDouble()) >= 1

    private fun possuiBomNivelRetornoSobrePatrimonio(roe: Double): Boolean =
        roe.compareTo(acaoConfig.minimoROE.toDouble()) >= 1

    private fun estaEmSetorPerene(setorDeOperacao: String): Boolean =
        acaoConfig.setoresParenes.contains(setorDeOperacao)

    private fun possuiBomNivelMargemLiquida(margemLiquida: Double): Boolean =
        margemLiquida.compareTo(acaoConfig.minimoMargemLiquida.toDouble()) >= 1

    private fun possuiBomNivelDividaLiquidaSobrePatrimonioLiquido(dividaLiquidaSobrePatrimonioLiquido: Double): Boolean =
        dividaLiquidaSobrePatrimonioLiquido.compareTo(0.00) >= 1 && dividaLiquidaSobrePatrimonioLiquido.compareTo(
            acaoConfig.maximoDividaLiquidaSobrePatrimonioLiquido.toDouble()
        ) < 1

    private fun possuiBomNivelDividaLiquidaSobreEbitda(dividaLiquidaSobreEbitda: Double): Boolean =
    ...
    private fun possuiBomNivelLiquidezCorrente(liquidezCorrente: Double): Boolean =
        liquidezCorrente.compareTo(1.00) >= 1
    ...
    ): Boolean = possuiBomNivelPrecoSobreLucro(precoSobreLucro) && possuiBomNivelPrecoSobreValorPatrimonial(
        precoSobreValorPatrimonial
    )

    private fun possuiBomNivelPrecoSobreValorPatrimonial(precoSobreValorPatrimonial: Double): Boolean =
    ...
    private fun possuiBomNivelPrecoSobreLucro(precoSobreLucro: Double): Boolean =
    ...
    private fun extrairDouble(texto: String): Double =
        if (StringUtil.isNullOrEmpty(texto) || texto.equals("-")) 0.00 else texto.trim().replace(",", ".")
            .replace("%", "").toDouble()
