...
class StockService(
    ...
    private fun getAnalisysWhenDoesNotYetExist(ticker: String): Mono<StockDTO> {
    ...
    private fun getAnalisysWhenAlreadyExist(ticker: String): Mono<StockDTO> {
    ...
    fun getExternalAnalisys(ticker: String): StockDTO {
        val responseDTO =
            ResponseDTO.parseMapToDto(br.com.company.stock.client.StockWebClient(acaoConfig).getContentFromAPI(ticker))
        ...
        val indicadorAlternativoPVP =
            if (indicadoresAlternativos?.get(1)?.Value_F == null) extrairDouble(valuation?.pvp.toString()) else extrairDouble(
                indicadoresAlternativos.get(1).Value_F!!
            )
        val precoSobreValorPatrimonial = extrairDouble(
            listOfNotNull(indicatorsTicker?.pvp, indicadorAlternativoPVP, valuation?.pvp).distinct().first().toString()
        )
    ...
    private fun possuiBomNivelMargemEbit(margemEbit: Double) =
        margemEbit.compareTo(acaoConfig.minimoMargemLiquida.toDouble()) >= 1
    ...
    private fun possuiBomPrecoEmRelacaoAoValorPatrimonial(precoSobreValorPatrimonial: Double) =
        possuiBomNivelPrecoSobreValorPatrimonial(precoSobreValorPatrimonial)
